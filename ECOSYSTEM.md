...
│  ─────────────────     │           │  ──────────────────────────  │
│  kb.json (V2.3/342)    │ validate  │  data/julian_all.json        │
│  match_rules()         │ empirical │  58K+ records (accuracy A-F) │
│  Typhoon LLM           │◄──────────│  Wikidata + Astrotheme + TH  │
│  prediction text       │  (planned)│  GitHub repo (CORS-free)     │
└────────────────────────┘           └─────────────────────────────┘
```
...
### 2. Empirical Validation (JULIAN → BIBLE) — planned
```
JULIAN data/julian_all.json:
  บุคคลสำคัญ (name, jd, birth_date, accuracy A-F, sources[])
  ↓ (planned — ยังไม่ implement)
BIBLE empirical_validator:
  get_data(jd) → planets[] → match_rules()
...
- **หน้าที่:** เก็บข้อมูลบุคคลสำคัญ + วันเกิด → ยืนยัน BIBLE rules ด้วย real-world data
- **Core:** `data/julian_all.json` (GitHub repo, CORS-free), 58K+ records, accuracy A-F
- **Dependency:** Wikidata SPARQL (primary) + Astrotheme/Wikipedia TH (enrichment)
- **Output:** empirical_p per rule → BIBLE quality score (planned — ยังไม่ implement)
...
### GUARD — Security + Performance Watchdog (cross-cutting)
...
- **Core:** `docs/GUARD_MISSION.md` (charter), risk register, SOPs, Phase 0-4 roadmap
...
### 🔄 Phase 1 — Data + KB (กำลังทำ)
- JULIAN: ✅ Automation running — 58K+ records, cron ทุก 6h
- JULIAN: Manual seed accuracy A/B/C — รอ user กรอก tools/julian_seed_input.html
- BIBLE: kb_v24-3 ✅ 290 rules — รอ Groq/Typhoon extraction แล้ว compare
- BIBLE: empirical_validator — รอ JULIAN มี data พอ
...
| Database | GitHub repo JSON | JULIAN storage (data/julian_all.json, CORS-free) |
...
JULIAN data/julian_all.json
  ├── BIBLE (empirical validation — planned)
  └── PLATFORM (personalized data สำหรับ chatbot — planned)
