...
# แก้จาก V3.3: เพิ่ม VERSION DISPLAY RULE — comment header bump เฉพาะไฟล์ที่แก้จริง
...
### VERSION DISPLAY RULE (source of truth)
```
Version ที่แสดงบน App คือสิ่งที่ถูกต้องเสมอ
Source of truth (3 ตัวนี้ต้องตรงกันเสมอ):
  APP_VERSION ใน script.js  ← หลัก
  CACHE_NAME ใน sw.js       ← ต้องตรง
  version.json              ← ต้องตรง
  !! ถ้าไม่ตรง = reload loop

comment header ไฟล์อื่น:
  bump เฉพาะเมื่อแก้ไฟล์นั้นจริง ไม่บังคับให้ตรงทุกไฟล์
```
...
4. bump comment header เฉพาะไฟล์ที่แก้จริง
