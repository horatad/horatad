...
- 🔲 เพิ่ม `validate_inputs()` — pos[] NaN check, ascSign range 0-11 → **GUARD T-05 / R-03** (defensive layer, GUARD priority audit ก่อน)
- 🔲 เพิ่ม `validate_matched()` — filter rule.p ว่าง, MIN_MATCHED_RULES threshold → **GUARD T-05** (รวม audit pipeline)
