...
- [ ] **Tank redesign Phase C** — visible UI refactor (picker popup + outer toggle + source-aware) — 🟢 **CSP gate ปลดแล้ว (BIG 05-30)** — ทำได้เลย (inline handlers refactor ทำตอน CSP enforce จริง ภายหลัง)
- [ ] **Phase 2 Step 1** — core/ Tier 1 (engine/lunar/varga/transit) split — spec: `docs/HORATAD_modules.md` §2 Tier 1 — 🟢 **CSP gate ปลดแล้ว (BIG 05-30)** — ทำได้เลย (split ไม่ผูกกับ CSP)
...
- [ ] [ทดลองใช้] CSP violations 1 wk → Phase 2 P2-A enforce — ⚠️ **moot (BIG 05-30):** `_headers` ไม่ทำงานบน GH Pages → CSP Report-Only ไม่เคย active จริง → observation window ไม่เคยเริ่ม. CSP enforce รอ Cloudflare Transform Rules (DEFERRED จน user base ขยาย). **ไม่ block Phase 2 Step 1 / Tank C อีกต่อไป**
- [ ] [BLOCKED] cloud sync — รอ server confirm
- [ ] [BLOCKED] QR URL privacy — รอ Option A/B/C → **GUARD R-07 owner**

### Handoff ล่าสุด
`handoffs/HORATAD_20260529_v2.md`
...
- [x] ✅ **Empirical layer** (2026-05-30) — `v3/kb_empirical.json` (7 SU×SA rules + lift/p_adj/n) จาก JULIAN · generator `workers/kb_empirical_build.mjs` · 📨 HORATAD ต้อง wire engine
...
### สถานะ (2026-05-29 — updated session v8)
...
- **🧬 Genealogy = research asset อันดับ 1** (ปีเตอร์ standing rule — รายงานก่อน record count)
  ...
  - **🆕 v8 `julian_genealogy_expand.mjs`** — ดึงญาติ frontier ที่ยังไม่อยู่ใน DB (precision≥วัน) → สายต่อเนื่องขึ้น · merge fold bucket `genealogy_expansion` (priority ต่ำสุด) · **รอ CI รันจริง** (sandbox 403)
  - **🆕 v8 research post 06** — "ต่อสายเครือญาติที่ขาดตอน" เข้า inbox (ผ่าน policy gate) + `--only` flag กัน resurrect
...
- [x] ✅ **BIBLE session**: apply SU×SA 16 rules → `v3/kb_empirical.json` (7 rules, 2026-05-30) · MR×MR/SA×MR/SU×MR ถูกตัดที่ต้นทาง · generator: `workers/kb_empirical_build.mjs`
...
## PLATFORM — Training Center + Chatbot + Content 🟢 Phase 3 LIVE (FB auto-post)
...
- **🟢 Phase 3 LIVE (2026-05-29)** — `fb_autopost.yml` (cron 08:30 ICT) โพสต์คิวถัดไปขึ้น Page อัตโนมัติ
  → `FB_PAGE_ID` + `FB_PAGE_TOKEN` ตั้งใน GitHub Secrets แล้ว (Page token) · EP01 โพสต์จริงแล้ว
  → bot ย้าย `scheduled → posted` + commit อัตโนมัติ (rebase+retry กัน duplicate)
  → ⚠ FB_PAGE_TOKEN verify อายุใน debug tool (~ปลาย ก.ค. 2026 ถ้าหมดต้อง refresh)
- **manual loop ยังใช้ได้** (backup): `tools/fb_post_helper.html` → ปุ่ม "โพสต์แล้ว" ย้าย `scheduled → posted` (ต้องใส่ Classic PAT)
...
### Handoff ล่าสุด
`handoffs/PLATFORM_20260529_v4.md`
...
## GUARD — Confidentiality · Integrity · Availability + Performance 🟢 Phase 1 6/7 · FB pipeline audited
...
### FB pipeline audit ✅ (session 4 — 2026-05-29)
- `docs/cia/fb_pipeline_audit_2026-05-29.md` — audit 5 FB workflow + 7 worker (SOP-03)
- ผ่านเกณฑ์พื้นฐาน ไม่มี critical/P0 · finding ใหม่ R-20..R-23
- แก้: `fb_post_manage.yml` +`contents:read` (R-21) · `julian_sync.yml` backfill `if: !cancelled()` (JULIAN req)
- `docs/SECRETS.md` +FB_PAGE_TOKEN/YOUTUBE secrets (R-20)

### Handoff ล่าสุด
`handoffs/GUARD_20260529_v4.md` (v1, v2, v3 archived)
...
|---|---|---|---|
| Horatad PWA | HORATAD V3.3.83 | script.js, v3/*, index.html | 🟡 Pre-launch — M1-M6 ✅ + province/time/column picker + recent chips + tag logic |
...
---
*อัปเดตล่าสุด: 2026-05-30 BIG session | CSP gate ปลดจาก HORATAD Phase 2 Step 1 + Tank C | handoffs/BIG_20260530_v1.md*
