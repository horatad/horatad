...
### สถานะ (2026-05-28 — updated session 8)
- Records: **122,788** (24.6%) — wikidata: 71,613 · musicbrainz: 51,175
- Events scraper: **704/1,331 batches** (53%) — รัน auto 4x/day, คาดเสร็จ ~2 วัน
...
### 📣 FB Content Pipeline (เพิ่ม 2026-05-28)
> JULIAN ต้องทำ 2 งานนี้ทุก session:
> 1. **ประมวลผล approved proposals** → `content/julian_proposals.json` status=approved → รัน analysis จริง → สร้าง caption ด้วยตัวเลขจริง → บันทึก `content/inbox/`
> 2. **เพิ่ม proposals ใหม่** → `node workers/julian_proposal_gen.mjs` → เพิ่ม planet pairs ใหม่

**Tool อนุมัติ:** `https://horatad.github.io/horatad/tools/julian_approval.html`
**Proposals file:** `content/julian_proposals.json` (10 pairs รออนุมัติ)
**Caption template:** ดูใน `workers/julian_proposal_gen.mjs`
...
- Claude (session) curate inbox → `content/scheduled/` → post to FB (Phase 3 — รอ page มี follower + Meta App Review)
- **Phase 3 ยังไม่พร้อม** — ระหว่างนี้: user copy โพสต์จาก `content/scheduled/` ขึ้น FB มือ
