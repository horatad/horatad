...
### Session lifecycle
1. **เริ่ม** → อ่าน `ECOSYSTEM.md` → `PROJECT_STATUS.md` → `handoffs/<PROJECT>_*.md` ล่าสุด → **รายงาน JULIAN data status** (ดูด้านล่าง)
...
### 📊 JULIAN data status — รายงานทุก session opening (mandatory)

ทุก session ไม่ว่า project ไหน ต้องรายงาน JULIAN automation progress ก่อนเสนองาน
(automation รันต่อเนื่อง — user อยากเห็นว่าข้อมูลโต/ช้า/หยุดไหมโดยไม่ต้องถาม)

**Current target: 100,000 records** (expanded จาก 50K — optimum sweet spot)

**Format 2 บรรทัด:**
```
📊 JULIAN: <N>/100,000 records (<%>) | <Q>/140 queries | velocity ~<avg>/run
📈 Phase <P>: <message> | next milestone: <M>K (eta ~<runs> runs)
```

**Milestone framework** (สำหรับ Phase indicator):
| Records | Phase | Message |
|---|---|---|
| < 10K | 1.early | Baseline building (basic coverage) |
| 10-25K | 1.mid | Statistical minimum reached (n=30/rule) |
| 25-50K | 1.late | BIBLE validation viable |
| 50-75K | 2.early | Expansion zone (Phase 2 enabled) |
| 75-100K | 2.late | Approaching optimum sweet spot |
| 100K+ | DONE | 🎉 Optimum reached — automation will stop |

**วิธีดึงตัวเลข:**
```bash
jq 'length' data/julian_all.json                           # records ปัจจุบัน
jq '.done_queries | length' workers/julian_progress.json   # queries เสร็จ
git log --format="%ai %s" -- data/julian_all.json | grep "JULIAN sync" | head -5
# ↑ 5 runs ล่าสุด — คำนวณ velocity + last run age + trend
```

**Velocity คำนวณจาก 2-3 runs ล่าสุดเท่านั้น** (run แรกเป็น baseline ใหญ่ — ไม่ representative)

**Flag เตือนถ้าเจอ:**
- ⚠️ last run > 12 ชม. → automation อาจหยุด/ติด lock → แจ้ง user ตรวจ workflow
- ⚠️ velocity drop > 50% เทียบ run ก่อน → queries เหลือ low-yield → ใกล้ ceiling
- ⚠️ records drop ระหว่าง run → sanity check fail → ตรวจ git log + restore จาก backup branch
- ✅ ข้าม milestone (25K/50K/75K/100K) → backup branch `backup/julian-data-NK` ถูกสร้างอัตโนมัติ
- 🎉 ≥ 100,000 → "JULIAN ครบ optimum — automation จะหยุดเอง, BIBLE validation พร้อมรัน"

**Decision suggestion (ทุก session opening เพิ่ม):**
- ถ้า velocity ปกติ + ยังไม่ครบ → "✅ ปล่อย automation รันต่อ"
- ถ้า velocity ดรอป + queries หมดเกือบหมด → "⚠️ พิจารณาเพิ่ม category queries หรือลด sitelinks filter"
- ถ้า data quality issue (audit drop เยอะ) → "🔴 พักการ enrich, ตรวจ parser ก่อน"

**Data loss protection (รู้ไว้ — workflow ทำให้):**
- Sanity check: records ลด > 5% → workflow abort
- Milestone backup: ข้าม 10K/25K/50K/75K/100K → `backup/julian-data-NK` branch ทุกครั้ง
- คืน data จาก backup: `git checkout backup/julian-data-50k -- data/julian_all.json`
