...
4. **Browser tool = proven path** สำหรับ Typhoon ทุก project — ใช้ pattern เดียวกับ `m0_hallucination_test.html`
...
### 📚 Project Memory Files — อ่านต้น session เขียนก่อนจบ session
...
- อัปเดตก่อนจบ session — เพิ่มสิ่งที่เรียนรู้ใหม่ใน session นั้น
- ห้าม rewrite เนื้อหาเดิม — **append only** (เพิ่มแถว/section ใหม่เท่านั้น)
- อัปเดต section "อัปเดต" ท้ายไฟล์ทุกครั้ง

### Session lifecycle
1. **เริ่ม** → อ่าน `ECOSYSTEM.md` → `PROJECT_STATUS.md` → `handoffs/<PROJECT>_*.md` ล่าสุด + **`handoffs/<PROJECT>_memory.md`**
   - **BIBLE session**: อ่าน `handoffs/BIBLE_memory.md` ด้วยเสมอ **ก่อนทำงานใดๆ**
...
4. **Commit** → push feature branch + ff main + backup branch (`backup/vX.Y.Z`)
   - **BIBLE session — ff main หลังทุก batch** (ไม่รอจบ session):
     BIBLE extraction session มักจบเพราะ context หมด ไม่ใช่เพราะ user สั่ง → ff main ต้องทำ **ทันทีหลัง commit แต่ละ batch** ขณะ context ยังเหลือ
     ```bash
     git push -u origin <feature-branch>
     bash scripts/admin/ff_main.sh <feature-branch>
     ```
     ถ้ายังมี batch ต่อไป → ทำต่อบน branch เดิมได้ แล้ว ff อีกครั้งหลัง batch นั้น
5. **ถ้ามี task เหลือเวลา** → ทำต่อตาม priority list (loop ข้อ 3-4)
6. **จบ session** → อัปเดต `handoffs/<PROJECT>_memory.md` ก่อน → แล้วจึงสร้าง `handoffs/<PROJECT>_<YYYYMMDD>_v<n>.md` + `PROJECT_STATUS.md`
   - เพิ่มสิ่งที่เรียนรู้ใหม่: bug patterns, architecture decisions, platform quirks, WHY LOG
   - **BIBLE session**: เพิ่มคำศัพท์ใหม่, pattern ใหม่, กฎที่ค้นพบ, ข้อสังเกตเกี่ยวกับภาษา

### 📚 BIBLE_memory — สิ่งที่เพิ่มโดยเฉพาะ (domain-specific)

นอกเหนือจากกฎทั่วไปด้านบน BIBLE_memory เพิ่มสิ่งเหล่านี้:
- คำศัพท์โหราศาสตร์ใหม่ / compound word ที่ต้องไม่ตัด
- pattern ของกฎที่พบซ้ำในหลายบท
- กฎที่พิสูจน์แล้วจาก case study (เช่น CH036)
- ข้อสังเกตเกี่ยวกับโครงสร้างประโยคในตำรา
- บทไหน high-value / บทไหนควรข้าม
- อัปเดต section 12 (Extraction Progress) ทุก session
