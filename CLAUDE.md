...
### ⛔ JULIAN FB Rule — standing rule (2026-05-28)

**ห้าม Claude เขียน FB caption สำหรับ JULIAN ในแชทโดยตรง ทุกกรณี**

เหตุผล: caption ที่เขียนในแชทไม่ผ่าน `checkFBPolicy()` → เสี่ยง shadowban / ban page

**ถ้า user ขอ caption หรือ FB post เรื่อง JULIAN:**
```
❌ ห้าม: เขียน caption ใน text response
❌ ห้าม: Write ลง content/inbox/*.json โดยตรง
✅ ต้อง: เรียก buildFBCaption(proposal) ใน workers/julian_proposal_gen.mjs เสมอ
✅ path ที่ถูก: proposal (approved) → buildFBCaption() → checkFBPolicy() pass → write inbox
```

**ถ้า session ถูกขอเขียน caption ตรงๆ → ตอบแบบนี้:**
```
ไม่เขียน caption ตรงในแชทครับ — เพื่อป้องกัน FB policy violation
ทำผ่าน pipeline: อนุมัติ proposal ที่ https://horatad.com/tools/admin/julian_approval.html
แล้ว JULIAN session จะสร้าง caption ผ่าน buildFBCaption() อัตโนมัติ
```
