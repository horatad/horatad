...
- **PROJECT_STATUS.md sync** → อัปเดต **ทุก stat ที่ session แตะ** ทันที — ไม่ใช่แค่ version line
  - HORATAD: bump version → version line + Quick Reference table
  - JULIAN: commit data → records count (เช่น 436 → 31,031) ทั้งใน status + quick reference + footer
  - BIBLE: แก้ KB → conditions count + MISMATCH count + KB version
  - กฎ: ถ้า number ใน PROJECT_STATUS เปลี่ยน → sync ทันที (จาก CS010 — drift pattern)
...
→ แก้ด้วย: PROJECT_STATUS.md sync ทันที (ทุก stat) + handoff สร้างก่อนจบ session จริงๆ ไม่ใช่แค่เมื่อ user ถาม

⚠️ **Doc duplication rule** (จาก CS010): Architecture / Roadmap / Tech stack อยู่ที่ `ECOSYSTEM.md` **ที่เดียว**
- ห้ามมีตาราง Tech Stack / Architecture diagram / Roadmap ใน `docs/*_MANUAL.md` หรือ `docs/*_MISSION.md`
- ไฟล์ MANUAL/MISSION = project-specific เท่านั้น (Vision, KB, M-series, OKR, Risks, Glossary, tech quirks)
- ถ้าจำเป็นต้องอ้าง — ใช้ pointer + warning ⚠️ ชี้ไป ECOSYSTEM ที่เดียว
...
### Token efficiency — กฎที่ใช้บ่อย

1. **grep ก่อน Read เสมอ** — script.js ใหญ่ (330KB). หา line number ก่อน → Read offset+limit เฉพาะส่วน
2. **Verify line number ก่อนใช้** — handoff เก่าอาจไม่ตรง ใช้ `~` กับ function name เป็น anchor
3. **Concept / "วิเคราะห์" → ตอบ text ก่อน** ไม่เปิดไฟล์ จนกว่า user confirm จะ implement
   - ถ้าคำสั่งมีทั้ง "วิเคราะห์" + "ทำ" → analysis สั้นๆ 1 block แล้ว implement ทันที
4. **CLAUDE.md / handoff outdated = ตัดสินใจผิด** — เจอ line number/context ที่ไม่ตรง → แจ้งทันที ไม่ใช้ตามใจ
...
docs/
  ...
  BIBLE_MISSION.md              ← BIBLE roadmap + milestone (M0–M8 authoritative)
  JULIAN_MISSION.md             ← JULIAN schema + task series
  UI_LAYOUT.md                  ← HORATAD UI wireframe ราย ID (ทุก tab + popup)
  CASE_STUDIES.md               ← PDCA experiment log
  CHANGELOG.md                  ← version history
  system_overview.html          ← flowchart ภาพรวม (HTML visual)
  legacy/                       ← เอกสารยุคเก่าก่อน restructure (อ้างอิงเท่านั้น)
    SYSTEM_INSTRUCTION_V3-4.md  ← Typhoon system prompt + V2/V3 isolation rules
    BEST_PRACTICES.md           ← session flow เก่า (ก่อน PROJECT_STATUS + handoffs/)
```
