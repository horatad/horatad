...
| ชื่อเล่น | **พี่ปีเตอร์ / Peter** (เรียก "พี่ปีเตอร์" เสมอ) |
...
**กฎ: เรียกผู้ใช้ว่า "พี่ปีเตอร์" ทุกครั้งเมื่อขึ้น session ใหม่**
...
## 🧭 Brand North Star — "โหราศาสตร์ที่ตรวจสอบได้" 🟡 (standing rule, 2026-05-29)

> ⭐ **ทุก session ที่ผลิต content / ออกแบบ UX / เขียน wording ต้องยึด section นี้** (PLATFORM · BIBLE · JULIAN · HORATAD)
> นี่คือ positioning ระดับแบรนด์ — ไม่ใช่แค่ marketing แต่กำหนดว่าเรา "พูดอะไรได้/ไม่ได้"

### Tagline 🟡 PROVISIONAL (รอตกผลึก — ปีเตอร์ยังไม่ฟันธง)
> **"โหราศาสตร์ที่ตรวจสอบได้"**
> *ไม่ต้องเชื่อก่อน — มาดูหลักฐานด้วยกัน*

**ทำไมเปลี่ยนจาก "ความเชื่อที่ตรวจสอบได้" (2026-05-29):**
- คำเดิมเอา "ความเชื่อ" (สากล กินถึงศาสนา) เป็นประธาน → โดยนัยดูหมิ่นศรัทธาคนอื่นที่ไม่ได้อยากพิสูจน์
- แก้: หดประธานเป็น **"โหราศาสตร์"** (ความเชี่ยวชาญของเรา) — เคลมแค่เรื่องตัวเอง ไม่ตัดสินใคร · โทนแบบกาลิเลโอ "มาดูสิ" ไม่ใช่ "ศรัทธาคุณผิด"

**⚖️ Unresolved tension (เหตุผลที่ tagline ยังไม่ final):**
- โหราศาสตร์ทุกแขนง claim หลัก = **"พยากรณ์อนาคต"** · แต่ "ตรวจสอบได้/falsifiable" คือมาตรฐานที่โหราศาสตร์ส่วนใหญ่สอบตก (JULIAN debunk เองหลาย rule — เหลือ SU×SA 16)
- ⚠️ ชู "ตรวจสอบได้" แรงเกิน → อาจทำลาย claim หลักของโหราศาสตร์เอง
- ทิศทางคลี่ (ยังไม่ final): "ตรวจสอบได้" = **วิธีการโปร่งใส + ซื่อสัตย์บอกว่าอะไรอ่อน/แข็ง** ไม่ใช่ "การันตีทำนายแม่น" · prediction = **probabilistic (แนวโน้ม)** ไม่ใช่ deterministic (ชะตากำหนด) · คุณค่า = ความหมาย/กรอบคิด + ส่วนที่รอดการกรอง

### ICP — "คนของเรา" = ผู้แสวงหาที่ไร้บ้านทางความเชื่อ
คนที่ติดอยู่ตรงกลางระหว่าง 2 ขั้ว:
- **ศรัทธาเก่าสลาย** — ไม่เชื่อสถาบัน/ศาสนา/หมอดูคลุมเครือ อีกแล้ว
- **ของใหม่ท่วมท้นแต่ไร้หลักฐาน** — เบื่อ wellness fad / conspiracy / self-help ลอยๆ

→ เขายัง**โหยหาความหมาย** แต่**ต้องตรวจสอบได้** · เคยถูกหลอกมาแล้วทั้งของเก่าและใหม่ → **หิวหลักฐานเป็นพิเศษ + ระแวง BS สูง**

### Positioning statement (Moore template)
> สำหรับ **คนที่ศรัทธาเก่าสลาย แต่ไม่ยอมเชื่ออะไรง่ายๆ อีก** ที่ยังโหยหาความหมายแต่ต้องตรวจสอบได้ —
> **โหราทาส** คือโหราศาสตร์ไทยแนวตรรกะ (TALS) ที่**พิสูจน์ด้วยข้อมูลจริง** ให้ความหมายโดยไม่ขอศรัทธาตาบอด ·
> ต่างจากหมอดู/เพจมูที่ขอให้ "เชื่อ" — เรา**เปิดให้ "ตรวจสอบ"**

### เราคือสะพาน (the gap we close)
| ข้อมูล/วิทยาศาสตร์เย็นชา | **โหราทาส (สะพาน)** | หมอดู/มูเตลู |
|---|---|---|
| จริง แต่ไร้ความหมายต่อชีวิต | โหราศาสตร์ + หลักฐาน | อบอุ่นมีความหมาย แต่ไม่เปิดให้ตรวจสอบ |

### 🔑 หลักการแกน — "ความซื่อสัตย์ = ตัวสินค้า" (ไม่ใช่แค่กันความเสี่ยง)
- โพสต์ที่บอก **"เราทดสอบแล้ว... pattern อ่อน ยังสรุปไม่ได้"** = โพสต์ที่ **on-brand ที่สุด** ไม่ใช่ความล้มเหลว
- ความกล้ายอมรับว่า "ยังไม่ชัด" = เครื่องมือสร้าง trust ที่ทรงพลังสุด — สิ่งที่หมอดู/เพจมูไม่เคยกล้าทำ
- **วินัย = การอยู่รอด**: กลุ่มนี้ให้โอกาสครั้งเดียว · overclaim ครั้งเดียว = เสีย trust ถาวร → **ทุกโพสต์ต้อง defend ได้ ห้าม cut corner ไล่ engagement**
- policy gate (n=, disclaimer, control group) = **เกราะป้องกันแบรนด์** ไม่ใช่ระเบียบราชการ

### FIT metric — วัดว่า "เล็งถูกคน" ไหม (สำคัญกว่ายอด ตอน stage เล็ก)
วัดจาก**คุณภาพ comment** ไม่ใช่จำนวน:
- ✅ ถูกกลุ่ม: "ขอดูกลุ่มควบคุมด้วย" · "n เท่าไหร่" · "ลองกับเคส X ไหม" (มาถกวิธีการ)
- ❌ ผิดกลุ่ม: "ขอเลขเด็ด" · "ดวงหนูเป็นไง" (ยังสื่อสารผิด)

### 🔬 Content engine — "โหราทาส Investigates" (flagship format)
เหตุการณ์ดัง → วิเคราะห์ดวงเชิงเปรียบเทียบด้วยข้อมูลจริง → ชวนคิด/ถก (newsjacking + JULIAN moat + audience-fit)

**3 hard gate (บังคับทุกครั้ง — ขาดข้อใดข้อหนึ่ง = ห้ามโพสต์):**
1. **Control group เสมอ** — กลุ่มเป้าหมาย vs กลุ่มควบคุม (n เล็กหา pattern เจอเสมอโดยบังเอิญ) · ถ้ากลุ่มควบคุมมี pattern พอกัน → โพสต์ความจริงนั้นอย่างกล้าหาญ
2. **Exploratory framing** — "ข้อสังเกต ไม่ใช่ข้อพิสูจน์" · ห้าม claim "TALS ทำนาย X ได้" (ไม่ผ่าน FDR/skeptic = ไม่ใช่ prediction rule)
3. **Taste gate** — เหตุการณ์สดกับคนยังมีชีวิต = เสี่ยงฉวยโอกาส · prefer historical · เหตุการณ์สดต้องระวังโทน

**Infra ที่รองรับแล้ว:** JULIAN (birthdate 129k + Wikidata P1196 manner-of-death + engine.js ดาวจร + สถิติ) · BIBLE (wording) · PLATFORM (เผยแพร่ผ่าน policy gate)
**ลีดโดย:** JULIAN (cohort builder + compute + control) → BIBLE (wording) → PLATFORM (publish)
...
เมื่อสร้างเอกสาร share ออกไป Google Docs ที่ต้องมี diagrams สวย (architecture, presentation, summary, design spec):

**ใช้ standard workflow:** `docs/templates/README.md`

```bash
# Render Mermaid → PNG + pandoc convert → DOCX
bash tools/render_doc.sh docs/<doc_name>
```

**Folder pattern:**
```
docs/<doc_name>/
  <doc_name>.md          ← markdown source
  diagrams/
    01_topic.mmd         ← Mermaid source (git-versioned)
    01_topic.png         ← generated (re-rendered when .mmd changes)
```

**กฎ:**
- ASCII art ใน Google Docs render เพี้ยน — ใช้ Mermaid + PNG เท่านั้น
- Mermaid sources (`*.mmd`) commit ด้วย (เปลี่ยน diagram = แก้ .mmd แล้ว re-render)
- Color palette มาตรฐาน → ดู `docs/templates/README.md` "Standard color palette"
- ใช้สำหรับ **share external** เท่านั้น — internal handoff/memory ใช้ markdown ปกติ (ASCII OK)

Example: `docs/bible_session_summary_2026-05-26/` (13 diagrams)
Dependencies: `mmdc` + `pandoc` (ดู README.md)
...
- **เมื่อรู้ scope แล้ว → ทำ 2 สิ่งทันที ก่อนทำงานใดๆ:**
  1. เขียน scope ลงไฟล์: `echo PROJECT > /home/user/horatad/.session_scope` (ห้ามใส่ quote, ห้าม chain &&, ต้องแยก call นี้ออก)
...
| **BIBLE** | `v3/kb*.json` · `v3/tals_*.json` · `v3/kb_skeletons.json` · `workers/kb_extract*` · `workers/groq*` · `workers/typhoon*` · `workers/claude_extraction*` · `tools/kb_*` · `tools/tals_*` |
| **JULIAN** | `workers/julian*` · `data/julian*` · `tools/julian*` |
| **NOK** | `v3/tts.js` |
...
### 📄 Doc management — Claude's job, not user's

**User ไม่ต้องดูแล doc** — Claude จัดการเองทั้งหมด:
- แก้ stale content, broken path, version drift → ทำเลย ไม่ต้อง approve
- ลบ doc ที่ redundant (duplicate ของ memory file / handoff) → archive ลง `handoffs/archive/` + บันทึกใน WHY LOG
- rewrite section ที่ outdated → ทำเลย บันทึกใน handoff DONE

**กฎเดียวที่ต้อง note ใน handoff:**
> "ลบ/ย้าย [ไฟล์] เพราะ [เหตุผล 1 บรรทัด]"

**Source of truth hierarchy (สูง → ต่ำ):**
1. Code (สิ่งที่ code ทำจริง)
2. `handoffs/<PROJECT>_memory.md` (domain knowledge สะสม — อ่านทุก session)
3. `ECOSYSTEM.md` (architecture + vision)
4. `handoffs/<PROJECT>_*.md` (task state ปัจจุบัน)
5. `CLAUDE.md` (rules สำหรับ Claude)

ไม่มี layer 6 — ถ้าต้องสร้างไฟล์ doc ใหม่ ให้ถามก่อนว่า layer ไหนรับ
...
- **ขอให้ user วาง secret (App Secret, access token, password, private key) ในแชท** — ดู Secret Handling Rule ด้านล่าง
- Skip CI (`--no-verify`) เว้นแต่ user สั่ง
- **เขียน FB caption ของ JULIAN ใน chat โดยตรง** — ดู JULIAN FB Rule ด้านล่าง

### 🔐 Secret Handling Rule — standing rule (2026-05-29)

**ห้าม Claude ขอให้ user วาง secret ใดๆ ในแชท ทุกกรณี**
(App Secret, access token, API key, password, private key, OAuth code)

เหตุผล: แชท Claude Code on web ถูกเก็บใน transcript ที่ server → secret ที่วางในแชท = หลุดจากการควบคุมของ user **ไม่มีกลไกปกปิด secret ในแชท** — โปรโตคอลที่ถูกต้องคือ "ไม่ส่ง secret ผ่านแชทตั้งแต่แรก"

**Secret ต้องอยู่ใน 1 ใน 3 ที่นี้เท่านั้น:**
| ที่เก็บ | ใช้กับ |
|---|---|
| GitHub Secrets (repo Settings → Secrets) | CI/workflow (`FB_PAGE_TOKEN`, ฯลฯ) |
| Sandbox env var | script ที่รันใน session (`TYPHOON_SERVER_KEY`, ฯลฯ) |
| เบราว์เซอร์/เครื่องของ user | one-time setup (เช่น แลก token ผ่าน address bar) |

**เมื่อ flow ต้องใช้ secret → Claude ทำแบบนี้:**
1. ให้ **URL/command template** ที่มี `APP_SECRET` `TOKEN` เป็น placeholder → user เติมค่าเองบนเครื่องตัวเอง
2. ผลลัพธ์ (เช่น Page token) → แนะนำให้ user เก็บใน GitHub Secrets ทันที ไม่วางกลับในแชท
3. ถ้า sandbox เรียก service ไม่ได้ (เช่น `graph.facebook.com` = Host not in allowlist) → บอกตรงๆ ว่า Claude รันให้ไม่ได้ ต้องทำฝั่ง user

**ถ้าเผลอเห็น secret ในแชทแล้ว → แนะนำ user reset/regenerate ทันที** (App Secret: Settings → Basic → Reset)



### ⛔ JULIAN FB Rule — standing rule (2026-05-28, updated 2026-05-29)

**ห้าม Claude เขียน FB caption สำหรับ JULIAN ในแชทโดยตรง ทุกกรณี**

เหตุผล: caption ที่เขียนในแชทไม่ผ่าน policy gate → เสี่ยง shadowban / ban page

**Content 2 ประเภท — ใช้ function ต่างกัน:**

| ประเภท | ตัวอย่าง | Function | ต้องมี |
|---|---|---|---|
| Statistical finding | ดาวอาทิตย์จรผ่านดาวเสาร์นาตาล | `buildFBCaption(proposal)` | n=, p-value, disclaimer, #TALS |
| Research / Behind-scenes | JULIAN คืออะไร, methodology, project milestone | `buildResearchCaption(post)` | disclaimer, #TALS (ไม่ต้องมี n=/p ทุก post) |

**Path ที่ถูกสำหรับ Statistical Findings:**
```
❌ ห้าม: เขียน caption ใน text response
❌ ห้าม: Write ลง content/inbox/*.json โดยตรง
✅ ต้อง: buildFBCaption(proposal) → checkFBPolicy() pass → write inbox
✅ path: proposal approved → julian_proposal_gen.mjs → inbox
```

**Path ที่ถูกสำหรับ Research / Behind-scenes:**
```
❌ ห้าม: เขียน caption ใน text response
❌ ห้าม: Write ลง content/inbox/*.json โดยตรง
✅ ต้อง: เพิ่ม post ใน workers/julian_research_gen.mjs → รัน script
✅ path: julian_research_gen.mjs → buildResearchCaption() → checkResearchPolicy() → write inbox
```

**📰 JULIAN Research Session Rule (standing rule 2026-05-29):**
> ทุก session ที่ทำงาน JULIAN research (เพิ่มข้อมูล, วิเคราะห์, สร้าง findings)
> ควรผลิต ≥1 research post เพื่อสร้าง content stream ควบคู่งานวิจัย
> วิธี: อัปเดต `workers/julian_research_gen.mjs` → เพิ่ม post → `node workers/julian_research_gen.mjs`

**ถ้า session ถูกขอเขียน caption ตรงๆ → ตอบแบบนี้:**
```
ไม่เขียน caption ตรงในแชทครับ — ต้องผ่าน policy gate เสมอ
Statistical finding → buildFBCaption() ใน julian_proposal_gen.mjs
Research/BTS content → buildResearchCaption() ใน julian_research_gen.mjs
```

### Deploy strategy — batch minor fixes (ประหยัด token)

**deploy ≠ กิน token** — แต่ version bump (หลายไฟล์ต่อ release — เช่น HORATAD 6 ไฟล์ ดู `docs/HORATAD.md`) มี overhead สูง
...
#### 1. grep ก่อน read เสมอ (ไฟล์ใหญ่ เช่น HORATAD `script.js`)
```bash
grep -n "function _foo" <file>   # หา line number ก่อน
Read <file> offset=N limit=40    # อ่านเฉพาะส่วนที่ต้องการ
```
...
| ต้อง re-read ไฟล์ใหญ่ (เช่น HORATAD `script.js` 200KB+) 3+ ครั้งใน task เดียว | finish task + checkpoint |
