...
**Format 1 บรรทัด:**
```
📊 JULIAN: <N>/50,000 records (<%>) | <Q>/137 queries | last run: +<Δ> records (<time ago>) | velocity ~<avg>/run
```
...
git log --format="%ai %s" -- data/julian_all.json | grep "JULIAN sync" | head -3
# ↑ 3 runs ล่าสุด — เอาเลข + records + timestamp มาคำนวณ velocity + last run age
```

**Velocity คำนวณจาก 2-3 runs ล่าสุดเท่านั้น** (run แรกเป็น baseline ใหญ่ — ไม่ representative)

**Flag เตือนถ้าเจอ:**
- ⚠️ ถ้า last run > 12 ชม. → automation อาจหยุด/ติด lock → แจ้ง user ตรวจ workflow
- ⚠️ ถ้า velocity drop > 50% เทียบ run ก่อน → queries เหลือ low-yield → คาดว่าใกล้ cap
- ✅ ถ้า ≥ 50,000 → "🎉 JULIAN ครบ target — automation จะหยุดเอง"
