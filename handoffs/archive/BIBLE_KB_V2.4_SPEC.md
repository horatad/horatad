...
## 2. Master Dictionary support ที่ต้องเสร็จก่อน

V2.4 schema พึ่ง `v3/master_dict.js` (แยกจาก engine.js — ดู section 11 architecture)
helpers — เสร็จแล้วใน session นี้:

| Helper | export from | สถานะ |
|---|---|---|
| `planet_relation(p1,p2)` → มิตร/ศัตรู/สมพล/ตน/ปกติ | `v3/master_dict.js` | ✅ ผ่าน R034 R035 R036 |
| `aspect_to_planet(pos,p1,p2)` → KUM/LENG/YOK/TRI/NONE | `v3/master_dict.js` | ✅ |
| `transit_to_natal_aspect(natalPos,transitPos,tIdx,nIdx)` | `v3/master_dict.js` | ✅ |
| `count_evil_lagna_aspects(pos,ascSign,minStrength)` | `v3/master_dict.js` | ✅ |
| `house_name_to_idx(name)` / `house_idx_to_name(idx)` | `v3/master_dict.js` | ✅ |
| `FRIEND_PAIRS / ENEMY_PAIRS / SAMPHON_PAIRS` | `v3/master_dict.js` | ✅ ground truth: kb chapter_08 + R034 R035 R036 |
| `ELEMENT_MAP` (ไฟ/ลม/น้ำ/ดิน) + `element_relation()` | `v3/master_dict.js` | ✅ ground truth: kb chapter_08 section 8.2 |
| `EVIL_PLANETS = [3,7,8,10]` (อังคาร เสาร์ ราหู มฤตยู) | `v3/master_dict.js` | ✅ |
| `HOUSE_NAMES_TH` ['','ตนุ','กฎุมพะ',...,'วินาศ'] | `v3/master_dict.js` | ✅ |
...
- เพิ่ม helpers ใน `v3/master_dict.js` — bump engine version → 3.1.0
...
- ground truth สำหรับ planet relations: kb chapter_08 (source PDF) + R034 R035 R036 ใน `v3/kb.json`
- master dictionary implementation: `v3/master_dict.js`
- audit findings: `handoffs/BIBLE_20260522_v2.md`
- previous schema: V2.3.0 (342 rules) ใน `v3/kb.json`

---

## 11. Architecture — Layer separation (added 2026-05-22)

```
v3/
├── engine.js         🔒 sacred — สุริยยาตร์โบราณ ห้ามแตะ logic คำนวณดาว
│                        get_data, get_j, _core, getStandards, KASET_MAP,
│                        EXALT_MAP, MAHACHAK_MAP, RACHA_MAP, STD_SCORE,
│                        HOUSE_SCORE, UNTO, Y_SUN/MON/OTH, MEAN_SPEEDS,
│                        PLANET_KEYS, PLANET_NAMES_TH, ZODIAC_TH,
│                        compute_std_score, get_tanu_lagna
│
├── master_dict.js    📖 rules-tier domain constants — โตได้
│                        FRIEND_PAIRS, ENEMY_PAIRS, SAMPHON_PAIRS,
│                        ELEMENT_MAP, EVIL_PLANETS, HOUSE_NAMES_TH,
│                        planet_relation, aspect_to_planet, element_relation,
│                        transit_to_natal_aspect, count_evil_lagna_aspects,
│                        house_name_to_idx, house_idx_to_name
│
├── interpretation.js  ⚙️ pipeline — build_natal_payload, compute_lagna_aspect,
│                        compose_local_prediction, evaluate_applies_when
│
├── typhoon.js         🔌 LLM connector + match_rules() + V2.4 reader
│
└── kb.json            📚 data — V2.3 ปัจจุบัน → V2.4 อนาคต
```

**Dependency rule (1-way):**
- `master_dict.js` import `engine.js` ได้ ✅
- `engine.js` ห้าม import `master_dict.js` ❌
- `interpretation.js` / `typhoon.js` import ทั้งสองได้ ✅

**ทำไม:**
- engine.js = ancient formula (สุริยยาตร์ Surya Siddhanta) — well-tested, ห้ามเปลี่ยน
- master_dict.js = domain knowledge ที่อาจเพิ่ม (relations, elements, อนาคต: เกษตรปุริฉะ, นวางค์, ฯลฯ)
- Audit diff: engine.js commit history ควรนิ่งระยะยาว; master_dict.js commit history คือ rules-base growth log
