...
## PENDING — 🔴 [BLOCKED] จาก JULIAN session (2026-05-21)
[ ] [BLOCKED] Julian Import: ปุ่ม "นำเข้าข้อมูล" ใน HORATAD ดึงข้อมูลจาก JULIAN D1
    dependency: ต้อง deploy Julian CF Worker ก่อน (workers/julian_wrangler.toml)
    flow: กดปุ่ม → fetch Julian Worker API → เก็บใน IndexedDB → ใช้ใน app
    schema: internet table (jd, name, event_label, country, confidence, time_utc)
    use case: search วันเกิดบุคคลสำคัญ, compare natal chart, validate rule empirically
