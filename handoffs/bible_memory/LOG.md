...
## 2026-05-27T00:00 — Voice chat grounding + taxonomy_keywords merge

**Session trigger:** Peter "ทำทุกอย่างที่แนะนำ"

### งานที่ทำเสร็จ

1. **Fix bible-qa.md Rule #3** — แก้ Rule #3 ที่ผิด (ภพ 1/2/7) → ถูก (chart_strength = f(ตนุลัคน์, กุมลัคนา)) พร้อมเพิ่ม Rule #4 + #5

2. **Voice chat anti-hallucination (v3tab.js)**:
   - `_vcSystemPrompt()`: เพิ่ม constraint "ตอบจากกฎที่ระบุเท่านั้น ถ้านอกขอบเขต → ตอบว่าไม่มีข้อมูล"
   - `_vcBuildRulesCtx()`: เพิ่ม [ดี]/[ร้าย]/[กลาง] + [domain] tags หน้า r.meaning ให้ Typhoon รู้บริบท

3. **taxonomy_keywords.json → docs/bible_extraction/**:
   - เพิ่ม vocabulary layer สำหรับ extraction toolkit + future query parsing
   - Contains: aspect_terms, quality_terms, planet_synonyms, prediction_verbs, polarity_signals, source_type_signals, tokenization_rules

4. **VOCAB.md**: merge sections ใหม่จาก taxonomy_keywords — planet_synonyms table, prediction_verbs, polarity_signals, aspect_terms, tokenization_rules

### Pattern learned
- Voice chat ใช้ r.meaning (full text) ซึ่งดีกว่า r.p (keywords) แต่ขาด domain/polarity context → Typhoon ตอบนอกขอบเขตได้ง่าย
- เพิ่ม [ดี][domain] tags = เลียนแบบ prediction tab pattern → constrain ได้ดีขึ้นโดยไม่ต้อง restructure send_chat()
- Prediction tab ยังใช้ kb.json เก่า (342 rules) ไม่ใช่ kb_tals.json — migration รอ HORATAD session (field schema ต่างกัน match_rules() ≠ matchRulesV24())

## 2026-05-27T12:00 — เสาร์ kaset correction + เกษตร vs เจ้าเรือน distinction

**Session trigger:** Peter ยืนยัน เสาร์ kaset=[9,10] และอธิบาย เกษตร≠เจ้าเรือน

### สิ่งที่เรียนรู้

**KEY DISTINCTION — ห้ามสับสน:**
- **เกษตร (quality position)** = quality state ที่ดาวได้รับเมื่ออยู่ในราศีนั้น → มีได้หลายดาวต่อ 1 ราศี
  - เสาร์(7) ได้เกษตรใน มกร(9) AND กุมภ์(10) พร้อมกัน
  - ราหู(8) ได้เกษตรใน กุมภ์(10) เช่นกัน
  - กุมภ์(10) จึงมีสองดาวได้เกษตรพร้อมกัน (เสาร์+ราหู)
- **เจ้าเรือน (house lord)** = ดาวที่ปกครองราศีนั้น สำหรับ house lord calculation → 1 ราศี = 1 เจ้าเรือนเท่านั้น
  - กุมภ์(10) = ราหู(8) เท่านั้น (ไม่ใช่เสาร์)
  - มังกร(9) = เสาร์(7) เท่านั้น

**"1 ดาว 1 ราศี" (TALS principle)** = apply to เจ้าเรือน เท่านั้น, ไม่ apply to เกษตร

**Correction of previous LOG entry (2026-05-28T15:00):**
- ผิด: เสาร์ kaset=[9] เท่านั้น
- ถูก: เสาร์ kaset=[9, 10] — มกร(9) primary + กุมภ์(10) secondary

### Files updated
- `v3/tals_planets.json` — เสาร์ kaset [9] → [9,10] + kaset_note
- `v3/tals_elements.json` — เสาร์ positions.kaset [9] → [9,10], anukaset [3] → [3,4], update notes + CORRECTED pending entry
- `v3/tals_quality_rules.json` — เทวีโชค derivation_pending → derivation confirmed (2026-05-28)
- `handoffs/bible_memory/PLANETS.md` — split "ดาวเกษตร (เจ้าเรือน)" section → แยก 2 tables: เกษตร (quality) + เจ้าเรือน (house lord)
- `.claude/hooks/session-start.sh` — BIBLE gate: embed INDEX.md foundational rules + last 3 LOG entries directly (no tool calls needed)

## 2026-05-27T14:00 — Transit architecture design + kb_transit.json สร้าง

**Session trigger:** Peter ออกแบบ 3-phase prediction (พื้นดวง / จร / Q&A)

### สิ่งที่เรียนรู้

**Architecture decision:**
- Phase 1: พื้นดวง top 20% (natal, high manifestation) — พูดออก
- Phase 2: จรทับพื้น หนัก 5 ดาว (MR/SA/RA/JU/MA → lagna/tanu) — พูดออก
- Phase 3: Q&A Typhoon keyword-driven — ปุ่มแยก
- จรทับจร: ตัดออก — ไม่ personal impact, ไม่ implement

**ch048 pattern (death case study):**
- จรทับพื้น = correct focus (ทุกเส้นใน ch048 เป็น จรทับพื้น)
- เส้นที่ซับซ้อน: transit → natal_planet (ไม่ใช่ lagna) ที่มีสายไปลัคนาอยู่แล้ว
- ภพวินาศ ของ natal planet ที่ถูกทับ = invert ผล (พฤหัสในภพวินาศ = ช่วยไม่ได้)
- Multiple transits converging = multiply severity (ไม่ใช่ add)
- Phase 3 complexity — ไม่ implement ตอนนี้

**kb_transit.json schema:**
- `transit_planet`: "MR"|"SA"|"RA"|"JU"|"MA"
- `aspect`: "KUM"|"LENG"|"YOK"|"TRI"
- `natal_target`: "LA" (lagna) | "TANU" (tanu_lagna planet)
- `polarity`: "bad"|"good"|"mixed"
- `severity`: 1-5
- source_type: INFERRED (derived from TALS planet natures)

### Files updated
- `v3/kb_transit.json` สร้างใหม่ — 40 rules
- `handoffs/HORATAD_20260526_v1.md` — append 📨 transit engine spec

### Key principle learned
"จรทับจร อย่างเดียว = background noise" — ต้องมี natal anchor ถึงเป็น personal impact

## 2026-05-27T15:00 — ch048 misinterpretation + transit rule review plan

**Session trigger:** Peter ชี้ว่า ch048 ใน kb_tals.json มีการ misinterpret

### สิ่งที่พบ

**R239 (ch048 case study) — INCOMPLETE extraction:**
Original text (4 elements):
1. เสาร์(7)จร ทับ เสาร์(7)เดิม → ตรีโกณลัคนา ✓ (captured)
2. พฤหัส(5)จร ทับ พฤหัส(5)เดิม → ในภพวินาศ ✓ (captured)
3. ศุกร์(6)จร ทับ เสาร์(7)เดิม (คู่ศัตรู) ← **MISSING**
4. เกตุ(9)จร เล็ง เสริมทวีคูณ ← **MISSING**

Rule ใน kb_tals.json จับได้แค่ 2/4 elements → incomplete rule, bukan wrong category

**Pattern ใหม่ที่พบ:**
- "transit X ทับ natal Y (X≠Y)" = จรทับพื้น (personal) แต่ยังไม่อยู่ใน kb_transit.json
- kb_transit.json ปัจจุบัน = เฉพาะ transit to LA/TANU
- transit to any natal planet (ที่ไม่ใช่ LA/TANU) = Phase 3

**misinterpretation ที่เคยทำ (ห้ามซ้ำ):**
- เรียก ch048 pattern ว่า "จรทับจร" → ผิด! ทุก element ใน ch048 = จรทับพื้น
- จรทับจร = 2 transit planets aspect กันโดยไม่มี natal anchor = ไม่ personal
- ศุกร์จรทับเสาร์เดิม = จรทับพื้น (transit X → natal Y, X≠Y) = ยัง personal อยู่

**INFERRED transit rules ที่ต้อง recheck next session:**
- R239: ขาด element 3 (ศุกร์จรทับเสาร์เดิม) + element 4 (เกตุจรเล็ง)
- R240: ราหูจรทับลัคนา + "พุธจรคู่ศัตรู" → ตรวจว่าหมายถึงพุธจรทับอะไรในพื้น
- R258: เสาร์จรเข้าภพปัตนิ → ถ้า text บอกตรงๆ ควรเป็น PRIMARY ไม่ใช่ INFERRED

**Pattern ที่ต้อง scan ใน review session:**
Rules ที่ extraction อาจ incomplete เพราะ text มีหลาย transit elements แต่จับได้บางส่วน
→ grep rules ที่ c/meaning มีคำว่า "จร" แล้วตรวจเทียบกับ chapter text ต้นฉบับ

### Files updated
- บันทึกใน LOG.md นี้ (review plan)
- Handoff: BIBLE_20260529 section PENDING (ต้องสร้างใหม่ next session)

## 2026-05-30T00:00 — 📌 PINNED: Empirical Layer (JULIAN→BIBLE) — kb_empirical.json

**Session trigger:** Peter "1" → apply SU×SA 16 valid rules → empirical_p ใน KB (ลบ MR×MR ก่อน)

### สิ่งที่ทำ
- สร้าง `workers/kb_empirical_build.mjs` (generator) + `v3/kb_empirical.json` (output, 7 rules)
- Input: `ml/models/julian_valid_rules.json` (JULIAN full-scan verdict, 16 valid SU×SA rows)
- Output: 7 empirical aspect-rules (group 16 rows by dist → 1 rule/มุม, events[] เก็บทุก event_type)

### 📌 Architecture decision — Empirical layer แยกไฟล์ (ไม่ยัดลง kb_tals.json)
```
kb_tals.json      = ground truth ตำรา 100CH (source_type=PRIMARY)  ← ไม่แตะ
kb_empirical.json = หลักฐานสถิติ JULIAN (lift/p_adj/n)             ← ชั้นใหม่
```
เหตุผล:
1. Provenance คนละชั้น — สถิติ ≠ ตำรา → กัน textbook ground-truth เปื้อน
2. บางมุมเกิน TALS model → flag `beyond_tals_model` ได้โดยไม่ทำลาย aspect taxonomy
3. North Star "ตรวจสอบได้" — ชั้นนี้คือ moat ที่ wording อ้าง n=/lift= ได้
4. Generator (ไม่พิมพ์มือ) → Zero-Human-Input + re-run ได้เมื่อ JULIAN เพิ่ม valid rules

### 🔬 ค้นพบสำคัญ (ต้องจำ — research narrative)
**มุมที่ TALS รับรอง (dist คู่: กุม/โยค/ตรีโกณ/เล็ง) = soft_award ทั้งหมด**
**สัญญาณ death (hard) มาจากมุมที่ TALS ตัดทิ้ง (dist คี่ 30°/90°/150°)**

| dist | มุม | TALS? | polarity | primary event |
|---|---|---|---|---|
| 0 | 0° กุม | ✓ | soft_award | award ↑15.6% (n=2225) |
| 1 | 30° | ✗ นอก TALS | hard_death | death ↑14.5% (n=2801) |
| 2 | 60° โยค | ✓ | soft_award | award ↑16.0% (n=4434) |
| 3 | 90° | ✗ นอก TALS | hard_death | death ↑12.6% (n=2823) |
| 4 | 120° ตรีโกณ | ✓ | soft_award | award ↑6.0% (n=3616) |
| 5 | 150° | ✗ นอก TALS | hard_death | death ↑18.1% (n=2684) |
| 6 | 180° เล็ง | ✓ | soft_award | award ↑11.9% (n=2080) |

→ TALS sign-aspect model จับ "benefic structure" ของ SU×SA ได้ แต่ "malefic structure" อยู่ในมุมที่ TALS ไม่รับรอง
→ ซื่อสัตย์: empirical แตกจาก model บางส่วน — เป็น finding ที่ต้องบอกตรงๆ ไม่ปิด

### Mapping reference (dist ราศี → TALS aspect)
dist 0=กุม(KUM) · 2=โยค(YOK,60°) · 4=ตรีโกณ(TRI,120°) · 6=เล็ง(LENG,180°) · คี่ 1/3/5 = นอก TALS
SU=อาทิตย์(id1) · SA=เสาร์(id7) · MR=มฤตยู(id0, en=Uranus)

### กลุ่มที่ JULIAN ตัด (ไม่เข้า empirical layer)
- MR×MR (19) = age proxy (มฤตยู period ~83yr ≈ อายุขัย → ตายแก่ ไม่ใช่โหราศาสตร์)
- SA×MR (5) + SU×MR (4) = inconsistent polarity (dist1 ไปคนละทาง)
- monitor: SA×SA (2) + SU×JU (2) = borderline รอ data เพิ่ม

### Cross-project impact
- 📨 HORATAD: engine ต้องเรียนรู้โหลด kb_empirical.json (ยังไม่ wire — note ใน handoff)
- 📨 JULIAN: empirical layer materialized แล้ว — task PROJECT_STATUS line 204 = DONE

### Re-run เมื่อไหร่
JULIAN เพิ่ม valid rules (เช่น SA×SA monitor ผ่าน, หรือ pair ใหม่) → `node workers/kb_empirical_build.mjs`
