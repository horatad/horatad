...
| 2026-05-27 | ML pipeline complete (features → CNN → explain → correlate) | (1) **Engine API call order**: `get_data(d, m, y, hr, mn)` — d,m,y ไม่ใช่ y,m,d (ผิดแล้ว 0 features). (2) **Engine raw unit**: `Math.trunc(rawPos[engineIdx] / 1800) % 12` = sign 0-11 (1800 steps/sign, NOT `val % 360 / 30`). (3) **getStandards API**: `getStandards(rawPos, engineIdx)` returns slash-sep string e.g. `'เกษตร/มหาอุจ'` — NOT object. Indices: 0=LA (return ''), 1=SU..10=MR. (4) **Feature planet order**: skip LA(0) → feature[0]=SU engineIdx=1, feature[9]=MR engineIdx=10. (5) **SHAP incompatible TF>=2.5**: DeepExplainer + GradientExplainer ทั้งคู่พัง (deep_tf.py shape mismatch) กับ GlobalMaxPooling2D → ใช้ `tf.GradientTape()` แทน. (6) **Windows cp1252**: `open(file, encoding='utf-8')` บังคับทุกไฟล์ Python ที่อ่าน Thai text. (7) **CNN 68.6% accuracy** (3-class, 33% baseline) — natal×transit sign interaction matrix มี signal จริง. (8) **MR (มฤตยู) dominant**: transit_MR conj natal_MR = death lift=2.26 (n=3198), transit_MR opp natal_MR = position_start lift=1.73 (n=2511) — empirical confirmation ของ TALS theory. MR ไม่มี quality mapping แต่มี event signal ชัดเจน. |
