...
| 2026-05-29 | genealogy = 3 แกน + tracking workers (strong point) | **แก้คำศัพท์ (ปีเตอร์ขอ):** "genealogy ด้านอาชีพ" จริงๆ เป็นคนละแกนกับเครือญาติ ต้องเรียกแยก 3 แกน: (1) **เครือญาติ family genealogy** = P22/P25/P26/P40/P3373 (สายเลือด/สมรส) → julian_genealogy_backfill→julian_family.json; (2) **สืบทอดตำแหน่ง office succession** = P39+P1365/P1366 (นายกฯ/ทหาร/ตำรวจ/กษัตริย์/โป๊ป/แชมป์หมากรุกโลก) → julian_succession_backfill→julian_succession.json; (3) **สายผู้ครองตำแหน่ง/แชมป์รายปี title lineage** = P166+P585 (นางงามจักรวาล/แชมป์มวยรายปี/รางวัลประจำปี) → **ใหม่** julian_title_lineage.mjs→julian_title_lineage.json. **gap ที่ปิด:** title lineage ไม่มี P1365/P1366 qualifier (ไม่ใช่ office) → ต้อง sequence เองจากปี: ดึง award+ปี → normalizeTitle ตัดปี 4 หลัก ('Miss Universe 1990/91' รวมเป็น 'Miss Universe') → เรียงปี → link ผู้ครองต่อเนื่อง (ข้ามคนครองซ้ำติดกัน, ต้อง ≥2 คนจึงเป็นสาย). **julian_genealogy_analyze.mjs** = tracking engine 3 metric: lineage depth (longest parent→child DAG + cycle guard, นับ root chain), clan (connected component ทุก relation), dynasty **by_position** (longest path ต่อตำแหน่ง — อ่าน succession+title_lineage รวมกัน). ทุก metric in-DB only (ทั้งสองฝั่งต้องมีดวง). output julian_genealogy_report.json. buildLineages + normalizeTitle เป็น pure export → offline test ด้วย --from-tuples. **⚠️ ยังต้อง wire เข้า workflow (GUARD):** เพิ่ม step julian_title_lineage.mjs (--resume --time-budget) + julian_genealogy_analyze.mjs ใน Job 2 + commit julian_title_lineage_checkpoint.json. |
| 2026-05-29 | ⭐ หลักโหราศาสตร์ที่ปีเตอร์สอน (paraphrase + persist — ปีเตอร์กำชับว่า Claude เข้าใจคำเป็น token อาจคลาด ห้ามลืม) | **(1) นาฬิกาปลุก = spurious accuracy policy:** โหราศาสตร์บอก "ช่วงเวลา/แนวโน้ม" ไม่ใช่จุดเป๊ะ — เหมือนเสียงปลุก คนตื่นก่อน/หลัง/ตอนเสียงดังก็ได้ + "คนถูกยิงไม่ตายทันที" เหตุการณ์มีช่วงผ่อนปรน. **ต้องใส่ในนโยบายวิจัย: beware spurious accuracy — ห้ามอ้างความแม่นเกินจริง.** นัยต่อปัญหาปฏิทิน Julian/Gregorian: ดาวเร็ว (จันทร์ ~13°/วัน) **ไม่ใช่ปัญหา** เพราะ event tolerance + ดาวช้าแทบไม่ขยับใน 10-13 วัน → calendar shift กระทบจำกัดมาก. **(2) คนพิเศษ > คนทั่วไป:** โหราศาสตร์เก่ง "แยกแยะคนไม่ธรรมดา" → DB บุคคลสำคัญ = **จุดแข็งตรงสนาม ไม่ใช่ bias ที่ต้องขอโทษ**. คนพิเศษจริง **ลัคนามีน้ำหนักน้อย** → birth-time gap (99.5% ไม่มีเวลา) ไม่ใช่ปัญหาใหญ่. **"ภพ (House) เล่าเรื่อง ไม่ได้บอกคุณภาพของเรื่อง — คุณภาพมาจากดาว"** → ดาวระดับราศี (ไม่ต้องใช้เวลาเกิด) เพียงพอสำหรับมิติคุณภาพ. **(3) era/cohort = mundane astrology (ของจริง):** เหตุการณ์ร่วมของคนแต่ละยุค **เป็นปรากฏการณ์จริงทางโหราศาสตร์** (เหตุผลที่โหราศาสตร์ภพมีอยู่) ไม่ใช่ noise ที่ลบทิ้ง — methodology ที่ดีต้อง **"แยก" natal ออกจาก mundane** ไม่ใช่ลบมั่ว. **(4) confirmatory + reverse คู่ขนาน:** แผนเดิม = พิสูจน์กฎ TALS กับ events แต่การสกัดกฎช้า (เพราะความเข้าใจ token/word/taxonomy/syntax จำกัด) → จึงรัน reverse-engineer (event→planet) คู่ขนานไปก่อน จะ bridge เมื่อ rule base พร้อม — **ติดป้ายแยก 2 กองจนกว่าจะรวม**. **(5) replication = ผู้พิพากษา spurious:** retest กลุ่ม sample ใหม่แล้วผลต่าง → เดิมคือ spurious; ผลที่ "อยู่" ข้าม sample = จริง. justify overall result ด้วย out-of-sample confirmation. |
| 2026-05-29 | TALS research framework ที่นำมาใช้กับ Julian DB | **(1) Two-tier research model (ปีเตอร์ articulate ครั้งแรก session นี้):** Julian DB มีสองระดับ utility ต่างกัน: [Tier 1] Quality Research = ใช้ 125,341 records (accuracy D, วันเกิดเพียงอย่างเดียว) → ทดสอบ Rule #5 (ดาวเด่น vs อาชีพ), quality correlations, สถิติคนดังมีดาวอุจ/เกษตรมากกว่าเฉลี่ยไหม — ทำได้เต็มที่เพราะ quality ใน TALS = sign-based (ไม่ต้องใช้เวลาเกิด); [Tier 2] Activation Research = ใช้ 614 records (accuracy C, มีเวลาเกิด) → ทดสอบ Rule #2 (lagna activation), ดาวสัมพันธ์ลัคนาของคนดังมี quality สูงกว่าดาวที่ไม่สัมพันธ์ไหม, event timing กับ lagna relationship. สองระดับนี้ต้องการ methodology ต่างกัน — อย่า mix กัน. **(2) Selection bias = จุดแข็ง ไม่ใช่ข้อเสีย:** Julian = บุคคลสำคัญทั้งหมด (Wikidata sitelinks filter) — ไม่ใช่ตัวแทนประชากรทั่วไป แต่ตรงกับสนาม TALS ที่ "เก่งแยกแยะคนพิเศษ" ใช้ตอบ: "configuration ไหนสัมพันธ์กับความสำเร็จ" ไม่ใช่ "ใครจะประสบความสำเร็จ" (ไม่มี control group). **(3) ยืนยัน sign-level quality = เพียงพอสำหรับ Tier 1 (ปีเตอร์ยืนยันอีกครั้ง):** ดาวระดับราศี (ไม่ต้องใช้เวลาเกิด) เพียงพอสำหรับมิติคุณภาพ เพราะ "ภพเล่าเรื่อง ดาวบอกคุณภาพ" — quality มาจาก sign ไม่มาจาก house. **(4) Subgroup property นัยต่อ research:** ถ้าทดสอบ Rule #2 กับ 614 records — transit ที่สัมพันธ์ลัคนา + natal ที่สัมพันธ์ลัคนา จะ automatically สัมพันธ์กันเอง (100%) ดังนั้นอย่า double-count ใน statistical model. |
| 2026-05-29 | pandoc+xelatex PDF generation tooling (TALS report) | **pandoc ใช้ได้ใน sandbox นี้** (ติดตั้งได้ผ่าน apt). xelatex สำหรับ Thai: ต้องใช้ ucharclasses + \setTransitionTo{Thai}{\thaifont} + **\setTransitionFrom{Thai}{\normalfont}** (ขาด setTransitionFrom → comma/colon ระหว่าง Thai text แสดงเป็น □ เพราะ Thai font ไม่มี Latin punctuation). mainfont = "Noto Serif" (Latin), thaifont = "Noto Serif Thai" (Thai chars only). pdfinfo จาก poppler-utils ใช้ตรวจ page count ได้. pdftotext ใช้ verify content ได้. |
| 2026-05-29 | PDF report tooling — sandbox constraints | **fpdf2 + pypdf + pdfminer ใช้ไม่ได้ใน sandbox** (ทุกตัวพึ่ง `cryptography` ที่มี rust binding พัง: `ModuleNotFoundError: _cffi_backend` + pyo3 PanicException). **ใช้ reportlab แทน** (pure python, pip install ได้, ไม่พึ่ง cryptography). ไม่มี: pandoc/latex/wkhtmltopdf/weasyprint/chrome/mmdc/matplotlib/ghostscript/pdftoppm → **rasterize/อ่าน PDF กลับไม่ได้เลยใน sandbox**. วิธี verify PDF โดยไม่เปิดดู: (a) instrument `_pagemin` track min-y ต่อหน้า → กัน overflow, (b) `fontTools.ttLib` เช็ค glyph coverage เทียบตัวอักษรไทยใน source → กัน tofu (Sarabun ครบ 62 ตัว). Thai font: `tools/assets/fonts/Sarabun-{Regular,Bold}.ttf` (commit แล้ว reproducible). generator: `tools/julian_report_gen.py` (reportlab canvas, origin bottom-left, track cursor y เอง). **บทเรียน process:** อย่าใส่ "memory append" ใน batch เดียวกับงานหลัก — ถ้า batch ถูก cancel กลางคัน memory จะหลุดแต่ commit อาจผ่าน → commit message ไม่ตรงเนื้อหา (เกิดจริง commit 4957e33). |
| 2026-05-29 | ancestor expansion worker + genealogy storage analysis (สำคัญต่อ scale) | **(1) backfill vs expand ต่างกัน:** `julian_genealogy_backfill` เก็บ *ความสัมพันธ์* ของคนใน DB (p/m/sp/ch/si) แต่ตัวญาติเองหลายคนไม่อยู่ใน DB → `genealogy.json` (in-DB only) ตัดทิ้ง → สายขาด. `julian_genealogy_expand.mjs` (ใหม่) ดึงญาติ frontier (ปรากฏใน family map แต่ไม่อยู่ใน dbQIDs) มาเป็น record เต็ม. **iterative deepening** — รันซ้ำ ญาติใหม่มี family ของตัวเอง = frontier รอบถัดไป. guardrail: precision≥11(วัน) เท่านั้น, source=`genealogy_expansion:Q...`, bound ต่อ run. exports: `collectFrontier(familyMap,dbQIDs)` + `bindingsToRecords(bindings)` (pure → offline test ด้วย `--from-bindings`). **(2) merge bucket discovery = hardcoded** — `julian_merge.mjs` อ่าน bucket ตามชื่อตายตัว (wikidata/coord/astrotheme/wiki_th/seed) → bucket ใหม่ **ต้องเพิ่มใน merge เอง** ไม่ auto. เพิ่ม `genealogy_expansion` ที่ `applyBase` **ก่อน wikidata** (priority ต่ำสุด — last-write-wins → canonical wikidata override ได้ถ้า person เดียวกันถูก scrape ตรงภายหลัง). **(3) JD epoch ใน julian_all.json = 730428** (engine.js get_j, Jan1 2000) **ไม่ใช่ astronomical 2451545.5** ที่ §2 schema เขียน ⚠️ แก้ไข: §2 ผิด — actual scraper/expand ใช้ 730428. **(4) genealogy storage = flat map ไฟล์เดียว `{QID:{...}}`** — `loadGenealogyIndex()` fetch ทั้งไฟล์เพื่อตอบคนเดียว. ปัญหา adjacency-list **ไม่ใช่ "หาหลายชั้น"** (พอโหลดเข้า RAM แล้ว traverse ancestor/descendant เร็ว O(depth)) แต่คือ **"โหลดทั้งกราฟก่อน"**. ทางแก้เมื่อโต = **shard ตาม clan** (connected component ที่ `genealogy_analyze` คำนวณอยู่แล้ว) → `genealogy/clan_NNNN.json` + `qid_to_clan.json` จิ๋ว → fetch มี bound, traverse ครบ in-clan offline (คำถามวิจัยเชิงสัมพันธ์เป็น within-clan ทั้งหมด). **threshold: shard เมื่อ genealogy.json > ~300-500KB หรือหลัง expansion รันใน CI จริง** (อย่า premature — flat map ตอนนี้ยังเล็ก). **(5) research_gen `--only <slug>` flag** — รันทั้ง array จะ resurrect โพสต์ที่ไป pipeline แล้ว (writeResearchPost เขียนทุกตัวใน array, filename=date+slug ทับของวันเดียวกัน); ใช้ `--only` เขียนเฉพาะโพสต์ใหม่. **(6) import side-effect:** `import ... from julian_proposal_gen.mjs` รัน top-level proposal-gen ทันที (เขียน julian_proposals.json) — idempotent ถ้าไม่มี finding ใหม่ แต่พึงระวังตอน import. |
