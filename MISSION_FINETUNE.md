...
### Benchmark tool (มีอยู่แล้ว)
ไฟล์: `thai-astro-api-compare.jsx` (React Artifact จาก Claude session)
- Tab 1: Speed & Quality — parallel fetch, วัด latency / TTFB / chars
- Tab 2: Hallucination Test — 6 ข้อมี ground truth, Claude ประเมินอัตโนมัติ

### Hallucination Test — 6 ข้อ seed (ขยายเป็น 20+ จาก KB)

| # | คำถาม | เฉลย |
|---|---|---|
| Q1 | ดาวพระเคราะห์ 7 ดวงคลาสสิกมีอะไรบ้าง | อาทิตย์ จันทร์ อังคาร พุธ พฤหัส ศุกร์ เสาร์ |
| Q2 | ดาวเจ้าเรือนราศีมังกรคือดาวอะไร | ดาวเสาร์ |
| Q3 | มหาทักษามีกี่หมวด หมวดร้ายที่สุดชื่ออะไร | 8 หมวด, กาลกิณี |
| Q4 | โหราศาสตร์ไทยใช้ระบบ Tropical หรือ Sidereal | Sidereal |
| Q5 | ดาวราหูมีตัวตนจริงหรือไม่ | ไม่มี เป็นดาวเงา (จุดตัดวงโคจร) |
| Q6 | ราศีใดที่ดาวพฤหัสเป็นเจ้าเรือน | ราศีธนู + ราศีมีน |
...
### 🔴 รอ user
- 🔴 สมัคร API Keys: Gemini (aistudio.google.com) + Groq (console.groq.com) + iApp (api.iapp.co.th)
- 🔴 รัน benchmark tool เทียบ latency + hallucination score

### 🔲 ต้องทำ (Claude)
- ⭐ 🔲 ขยาย hallucination test จาก 6 → 20+ ข้อ จาก KB rules ที่มี conditions verified
- 🔲 สร้าง cross-validation tool: โหลด kb_yaml_import.json → ส่ง Gemini + Groq + Typhoon → เทียบกับ known rules → แสดง accuracy per LLM
